...
public class WheelchairShirtController : MonoBehaviour
{
    // .json log & Video
    private float globalStartTime;
    private List<WShirtStepLog> stepLogs = new List<WShirtStepLog>();
    ...
    void Start()
    {
        ...
        SetAllRenderersEnabled(shirt, false);
        SetAllRenderersEnabled(Cloth, true);
        SetAllRenderersEnabled(ClothGroup, true);

        StartCoroutine(RunShirtSequence());
    }

    // === Core Shirt Sequence ===
    IEnumerator RunShirtSequence()
    {
        ...
        if (recorder != null)
            recorder.BeginRecording("WheelchairShirt");

        yield return StartCoroutine(RunAndLogStep(OrientTShirt(), "OrientTShirt"));
        yield return StartCoroutine(RunAndLogStep(TShirtOnChairDressLeftArm(), "TShirtOnChairDressLeftArm"));
        yield return StartCoroutine(RunAndLogStep(TShirtOnChairDressRightArm(), "TShirtOnChairDressRightArm"));
        yield return StartCoroutine(RunAndLogStep(TShirtOnChairTShirtOverHead(), "TShirtOnChairTShirtOverHead"));
        yield return StartCoroutine(RunAndLogStep(TShirtOnChairPullTShirtDown(), "TShirtOnChairPullTShirtDown"));
        ...
        Debug.Log("✅ All shirt steps complete.");
    }
    ...
    // === Step 2 ===
    IEnumerator TShirtOnChairDressLeftArm()
    {
    ...
    // === Step 3 ===
    IEnumerator TShirtOnChairDressRightArm()
    {
    ...
    // === Step 4 ===
    IEnumerator TShirtOnChairTShirtOverHead()
    {
    ...
    // === Step 5 ===
    IEnumerator TShirtOnChairPullTShirtDown()
    {
    ...
    IEnumerator FinalAdjustments()
    {
        ...
        SetAllRenderersEnabled(shirt, true);
    ...
    IEnumerator RunAndLogStep(IEnumerator coroutine, string taskName)
    {
        ...
        WShirtStepLog log = new WShirtStepLog
        {
    ...
    void SaveLogToJson()
    {
        // /home/qiandaoliu/Shirt/JSON
        string baseDir = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
            "WheelchairShirt", "JSON"
        );
        ...
        do
        {
            dir = Path.Combine(baseDir, $"shirt_{index:000}");
            index++;
        }
        ...
        Debug.Log($"✅ Shirt log saved to: {path}");
    }

    // Specific tools for Shirt task
