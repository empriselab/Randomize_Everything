...
public class ToiletController : MonoBehaviour
{
    ...
    private List<ToiletStepLog> stepLogs = new List<ToiletStepLog>();
    ...
    void Start()
    {
        ...
        StartCoroutine(RunToiletSequence());
    }

    // === Core Toilet Sequence ===
    IEnumerator RunToiletSequence()
    {
        yield return new WaitForSeconds(3.0f);

        globalStartTime = Time.time;
        if (recorder != null)
            recorder.BeginRecording("Toilet");

        yield return StartCoroutine(RunAndLogStep(ToiletingPanPanOn(), "ToiletingPanPanOn"));
        yield return StartCoroutine(RunAndLogStep(ToiletingPanPanOff(), "ToiletingPanPanOff"));
        ...
        Debug.Log("✅ All toilet steps complete.");
    }

    // === Step 1 ===
    IEnumerator ToiletingPanPanOn()
    {
        yield return new WaitForSeconds(0.2f);
        yield return StartCoroutine(MoveIKToAboveKinova());
        yield return StartCoroutine(MoveWheelchairTo(WheelChairPos1));

        yield return StartCoroutine(MoveIKTargetTo(ik1, 5.0f));
        yield return new WaitForSeconds(1.2f);

        yield return StartCoroutine(RunParallel(
            MoveIKTargetTo(ik2, 8.0f),
            MoveTransformToTarget(toilet, ik3, 8.0f)
        ));
    ...
    // === Step 2 ===
    IEnumerator ToiletingPanPanOff()
    {
        yield return new WaitForSeconds(0.5f);

        yield return StartCoroutine(RunParallel(
            MoveIKTargetTo(ik1, 8.0f),
            MoveTransformToTarget(toilet, ik4, 8.0f)
        ));
    ...
    IEnumerator RunAndLogStep(IEnumerator coroutine, string taskName)
    {
        ...
        ToiletStepLog log = new ToiletStepLog
        {
    ...
    void SaveLogToJson()
    {
        ...
        do
        {
            dir = Path.Combine(baseDir, $"toilet_{index:000}");
            index++;
        }
        ...
        Debug.Log($"✅ Toilet log saved to: {path}");
    }
